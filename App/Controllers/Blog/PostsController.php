...
public function save($id)
{
    ...
    if ($this->isValid($id)) {
        ...
        $json['redirectTo'] = $this->url->link('/posts');
    } else {
        // it means there are errors in form validation
        $json['errors'] = $this->validator->flattenMessages();
    }
...
private function form($post = null)
{
    if ($post) {
        ...
        $data['action'] = $this->url->link('/posts/save/' . $post->id);
        $data['heading'] = 'Edit ' . $post->title;
    } else {
        // Adding form
        $data['target'] = 'add-post-form';
        $data['action'] = $this->url->link('/posts/submit');
    ...
    return $this->view->render('posts/form', $data);
}
